...
app.get("/musicians", async (req, res, next) => {
    ...
    //! Manage our if conditional in a one line ternary
    const musicians = await Musician.findAll({
        ...
        limit: getAll ? undefined : size,
        offset: getAll ? undefined : size * (page - 1),
    });

    return res.json(musicians);

    //! Pass limit and offset only in the else case
    // // if (page === 0 || size === 0) {
    // if (getAll) {
    //     const musicians = await Musician.findAll({
    //         order: [["lastName"], ["firstName"]],
