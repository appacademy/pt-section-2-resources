...
describe("Bonus: isPrimary Spec", async () => {
    ...
    describe("Check for Primary colors", () => {
        it("Test primary colors", async () => {
            await expect(models.Color.create({ name: "Red", isPrimary: true }))
                .to.be.fulfilled;
            const red = await models.Color.findOne({
                where: {
                    name: "Red",
                },
            });
            await expect(models.Color.create({ name: "Blue", isPrimary: true }))
                .to.be.fulfilled;
            const blue = await models.Color.findOne({
                where: {
                    name: "Blue",
                },
            });
            await expect(
                models.Color.create({ name: "Yellow", isPrimary: true })
            ).to.be.fulfilled;
            const yellow = await models.Color.findOne({
                where: {
                    name: "Yellow",
                },
            });
        ...
        it("Test non-primary colors", async () => {
            await expect(
                models.Color.create({ name: "Green", isPrimary: false })
            ).to.be.fulfilled;
            const green = await models.Color.findOne({
                where: {
                    name: "Green",
                },
            });
        ...
        it("Checks default value of isPrimary to be false", async () => {
            await expect(models.Color.create({ name: "Black" })).to.be
                .fulfilled;
            const black = await models.Color.findOne({
                where: {
                    name: "Black",
                },
            });
        ...
        it("Check allowNull of isPrimary", async () => {
            await expect(
                models.Color.create({ name: "White", isPrimary: null })
            ).to.be.rejected;
        });

        // it("Checks default value of isPrimary to not be false when passed a primary color", async () => {
