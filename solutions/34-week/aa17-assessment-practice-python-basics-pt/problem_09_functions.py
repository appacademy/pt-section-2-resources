...
class PizzaProcessor():
  ...
  def pizza_pick(self, name, crust_type, num_toppings):
    if isinstance(name, str) and isinstance(crust_type, str) and isinstance(num_toppings, int) and num_toppings >= 0 and num_toppings <= 10:
      return f"{name.capitalize()} ordered a {crust_type} crust pizza with {num_toppings} toppings!"
    else:
      return self._error
  ...
  def pizza_prepper(self, *num_toppings):
    ...
    for order_num, num_topping in enumerate(num_toppings):
      if isinstance(num_topping, int) and num_topping >= 0 and num_topping <= 10:
        self._pizzas_made += 1

        if order_num % 2 == 0:
          pizza_orders[str(order_num)] = { "crust": "thin", "toppings": num_topping }
        else:
          pizza_orders[str(order_num)] = { "crust": "stuffed", "toppings": num_topping }
      else:
        return self._error
  ...
  def pizza_printer(self, **pizza_orders):
    ...
    print(pizza_orders.items())
    for order_num, pizza in pizza_orders.items():
